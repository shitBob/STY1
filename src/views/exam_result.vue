...
/* From Uiverse.io by sabbircoder07 */ 
   <div class="results-summary-container">
      ...
      <div class="results-summary-container__result">
        <div class="heading-tertiary">正确个数</div>
        <div class="result-box">
          <div class="heading-primary">{{$router.currentRoute.value.params.correctNum}}</div>

          <p class="result">题目总数:{{ $router.currentRoute.value.params.tolnum }}</p>
        </div>
        <div class="result-text-box">
          <div class="heading-secondary" v-if="Number(router.currentRoute.value.params.correctNum) / Number(router.currentRoute.value.params.tolnum) >= 0.8">优秀</div>
          <div class="heading-secondary" v-else-if="Number(router.currentRoute.value.params.correctNum) / Number(router.currentRoute.value.params.tolnum) >= 0.6">合格</div>
          <div class="heading-secondary" v-else-if="Number(router.currentRoute.value.params.correctNum) / Number(router.currentRoute.value.params.tolnum) < 0.6">不及格</div>
          <p class="paragraph" v-if="Number(router.currentRoute.value.params.correctNum) / Number(router.currentRoute.value.params.tolnum) >= 0.8" >
            犀利，准确的答案，深刻的分析，精彩的展示，令人赞叹的成绩。
          </p>
          <p class="paragraph" v-else-if="Number(router.currentRoute.value.params.correctNum) / Number(router.currentRoute.value.params.tolnum) >= 0.6" >
            好野，答对了一些题目，但还有部分题目需要加强，再接再厉。
          </p>
          <p class="paragraph" v-else-if="Number(router.currentRoute.value.params.correctNum) / Number(router.currentRoute.value.params.tolnum) < 0.6" >
            差些少窝，答错了很多题目，需要加强学习，道阻且长，加油！。
          </p>
        </div>
